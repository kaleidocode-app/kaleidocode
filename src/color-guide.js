...
export function createColorGuide(colorThemeName, sortedColors, n) {
    const guideFrameHeight = Math.ceil(sortedColors.length / COMPONENT_WRAP_COUNT) * (colorFrameHeight + gap) - gap;
    ...
    const borderFrame = createBorderFrame(colorThemeName, guideFrameWidth, guideFrameHeight, borderSize, xOffset);
    const innerGuideFrame = createInnerGuideFrame(borderFrame, borderSize, guideFrameWidth, guideFrameHeight);
    let colorFrameX = 0;
    let colorFrameY = 0;
    let groupCount = 0;
    ...
    sortedColorsWithLeadingBlocks.forEach((item, colorI) => {
        if (typeof item === 'string') {
            ...
            const colorFrame = createColorFrame(styleIndicator + fullColorName, innerGuideFrame, colorFrameWidth, colorFrameHeight, colorFrameX, colorFrameY);
            const fillStyleId = createStyle(colorThemeName, fullColorName, colorValue);
            const solidColorRect = createSolidColorRect(colorFrame, fillStyleId, colorFrameWidth, rectHeight);
            ...
            //label frame
            const labelFrame = createLabelFrame(colorFrame, solidColorRect, hexCodeFrame, labelFrameHeight);
            // The label
    ...
    borderFrame.resizeWithoutConstraints(guideFrameWidth + borderSize, colorFrameY + colorFrameHeight + borderSize);
...
//create colorFrame
function createColorFrame(name, guideFrame, colorFrameWidth, colorFrameHeight, colorFrameX, colorFrameY) {
...
function createSolidColorRect(colorFrame, newStyleId, colorFrameWidth, rectHeight) {
...
function createLabel(labelFrame, fullColorName) {
    let colorGroup, colorName;
...
function createGroupLeadingBlock(groupName, colorFrameWidth, colorFrameHeight, innerGuideFrame, x, y) {
    ...
    if (groupTitle.characters.length >= 20) {
        for (var i = 0; i <= groupTitle.characters.length; i++) {
            var currentCharacter = groupTitle.characters.charAt(i);
...
function getColorNames(fullName) {
...
export function createStyle(colorThemeName, fullColorName, colorValue) {
